...
-- SAFER migration: Only add missing owner memberships where the user still exists
-- First, delete any workspaces that have invalid owner_ids (users that don't exist)
DELETE FROM public.workspaces 
WHERE owner_id NOT IN (
  SELECT id FROM auth.users
);

-- Now add missing owner memberships for valid workspaces
