...
-- Add product_id column to prompts table (nullable for now)
ALTER TABLE prompts ADD COLUMN product_id UUID;

-- Add foreign key constraint for product_id in prompts
ALTER TABLE prompts ADD CONSTRAINT prompts_product_id_fkey 
FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE;

-- Add foreign key constraint for product_id in epics  
ALTER TABLE epics ADD CONSTRAINT epics_product_id_fkey 
FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE;
...
-- If a prompt has an epic_id, ensure it's associated with that epic (product_id remains null)
...
-- Create trigger to update updated_at for prompts when product_id changes
